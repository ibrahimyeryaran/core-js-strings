...
function findLongestWord(sentence) {
  ...
  [...sentence].forEach((value, len) => {
    if (!space) {
      currectWord.push(value);
    } else {
      if (currectWord.length > answ.length) {
        answ = [...currectWord];
      }
      currectWord = [];
    }
...
function reverseWords(str) {
  ...
  [...str].forEach((value, len) => {
    if (!space) {
      currectWord.push(value);
    } else {
      answ.push(...currectWord.reverse());
      answ.push(' ');
      currectWord = [];
    }
    if (len + 1 === str.length) {
